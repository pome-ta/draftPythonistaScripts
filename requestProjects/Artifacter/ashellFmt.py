...
class objc_util_loader(object):  # objc_util installer
  def __int__(self):
    with open(os.path.join(os.getcwd(), 'objc_util.py'), 'w', encoding='utf-8') as f:
      # f.write(_=base64String=_.decode(errors='ignore'))
      pass
    os.makedirs(os.path.join(os.environ.get('HOME'), 'Library', 'lib', 'python3.11', 'site-packages'), exist_ok=True)
    shutil.move(os.path.join(os.getcwd(), 'objc_util.py'),
                os.path.join(os.environ.get('HOME'), 'Library', 'lib', 'python3.11', 'site-packages', 'objc_util.py'))
...
try:
  from objc_util import ObjCClass, ObjCInstance, create_objc_class, on_main_thread, sel, CGRect
except ImportError:
...
class Artifacter(object):
  ...
  def Initialization(self):
    ...
    try:
      self.player_data = json.loads(urllib.request.urlopen(
        urllib.request.Request('https://enka.network/api/uid/{}'.format(self.uid),
  ...
  def mainoption(self):  # 聖遺物のメインオプション情報
    return {"会心ダメージ": {
      ...
      "ov": ["21.8", "22.5", "23.3", "27.2", "28.0", "28.8", "29.5", "30.3", "31.1", "32.6", "33.4", "34.2", "35.0",
             "35.7", "36.5", "35.8", "37.3", "38.1", "38.8"]
    },
      "会心率": {
        ...
        "ov": ["10.9", "11.3", "11.7", "13.6", "14.0", "14.4", "14.8", "15.2", "15.6", "16.3", "16.7", "17.1", "17.5",
               "17.9", "18.3", "18.7", "19.1"]
      },
      "元素チャージ効率": {
        ...
        "ov": ["18.1", "18.8", "19.4", "22.7", "23.3", "24.0", "24.6", "25.3", "25.9", "27.2", "28.5", "27.8", "29.8",
               "31.1", "32.4"]
      },
      "防御パーセンテージ": {
        ...
        "ov": ["20.4", "21.1", "21.9", "25.5", "26.2", "27.0", "27.7", "28.4", "29.2", "30.6", "31.3", "32.1", "32.8",
               "33.5", "34.3", "35.0", "35.7"]
      },
      "攻撃パーセンテージ": {
        ...
        "ov": ["16.3", "16.9", "17.5", "20.4", "21.0", "21.6", "22.2", "22.7", "23.3", "24.5", "25.1", "25.7", "26.2",
               "26.8", "27.4", "25.6", "28.0", "28.6", "29.2"]
      },
      "HPパーセンテージ": {
        "16.3": [[4.7, 5.8, 5.8], [4.1, 4.1, 4.1, 4.1]],
        "16.9": [[5.3, 5.8, 5.8], [4.1, 4.1, 4.1, 4.7]],
      ...
      "HP": {
        ...
        "ov": ["837", "866", "896", "1046", "1076", "1105", "1135", "1165", "1195", "1255", "1285", "1315", "1344",
               "1374", "1404", "1434", "1464", "1494"]
      },
      "攻撃力": {
        ...
        "ov": ["54", "56", "58", "68", "70", "72", "74", "76", "78", "82", "84", "86", "88", "89", "91", "93", "95",
               "97"]
      },
      "防御力": {
        ...
        "ov": ["65", "67", "69", "81", "83", "86", "88", "90", "93", "97", "100", "102", "104", "106", "109", "111",
               "113", "116"]
      },
      "元素熟知": {
        "65": [[19, 23, 23], [16, 16, 16, 16]],
        ...
        "ov": ["65", "68", "70", "82", "84", "86", "89", "91", "93", "98", "100", "103", "105", "107", "110", "112",
               "114", "117"]
      }
    }
  ...
  def locale(self):  # 読み込んだデータの日本語化
    LJsn = json.loads(urllib.request.urlopen(
      urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/loc.json',
                             headers={'User-Agent': UsrAgn})).read().decode(errors='ignore'))
  ...
  def character_json(self):  # キャラクター情報の取得
    return json.loads(urllib.request.urlopen(
      urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/characters.json',
                             headers={'User-Agent': UsrAgn})).read().decode(errors='ignore'))
  
  def character_pfps_json(self):  # キャラクターのアイコン取得
    return json.loads(urllib.request.urlopen(
      urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/pfps.json',
                             headers={'User-Agent': UsrAgn})).read().decode(errors='ignore'))
  ...
  def create_buildcard(self, avaterinfo, score_state):
    ...
    try:  # キャラクタースキルセットIDを判別。
      if len('{}'.format(avaterinfo['skillDepotId'])) == 3:  # キャラクタースキルセットIDが３なら以下を実行
        character_id = '{}-{}'.format(avaterinfo['avatarId'],
                                      avaterinfo['skillDepotId'])  # キャラクタースキルセットIDは[avatarid]-[skillDepotId]という形式で取得できる
        character_skill_icons = ['{}.png'.format(list(self.characters_json[character_id]['Skills'].values())[2]),
                                 ...
                                 '{}.png'.format(list(self.characters_json[character_id]['Skills'].values())[
                                                   1])]  # キャラクター天賦画像(画像上半分の真ん中にある細長い長方形の上半分)
      else:
        character_id = '{}'.format(avaterinfo['avatarId'])
        character_skill_icons = ['{}.png'.format(skill) for skill in self.characters_json[character_id][
          'Skills'].values()]  # キャラクター天賦画像(画像上半分の真ん中にある細長い長方形上半分)
    except:
      character_id = '{}'.format(avaterinfo['avatarId'])
      character_skill_icons = ['{}.png'.format(skill) for skill in self.characters_json[character_id][
        'Skills'].values()]  # キャラクター天賦画像(画像上半分の真ん中にある細長い長方形の上半分)
    ...
    try:
      character_icon = '{}.png'.format(
        self.characters_json[character_id]['Costumes']['{}'.format(avaterinfo['costumeId'])]['sideIconName'].replace(
          'UI_AvatarIcon_Side_', 'UI_Costume_'))  # キャラクターがコスチュームを付けていた場合の対応
    except:
      character_icon = '{}.png'.format(
    ...
    try:
      ...
      character_const = -1
    character_const_icons = ['{}.png'.format(cicon) for cicon in self.characters_json[character_id][
      'Consts']]  # キャラクター凸画像(画像上半分の真ん中にある細長い長方形の下半分の鍵アイコンが入ってる部分のアイコン画像)
    ...
    character_element_buff_json = {'30': '物理ダメージ', '40': '炎元素ダメージ', '41': '雷元素ダメージ',
                                   '42': '水元素ダメージ', '43': '草元素ダメージ', '44': '風元素ダメージ',
                                   '45': '岩元素ダメージ', '46': '氷元素ダメージ'}  # キャラクターの元素ダメージの種類
    ...
    character_element_buff_name = '{}'.format(
      character_element_buff_json[character_element_max_buff_key])  # 元素ダメージの読み方を設定
    ...
    artifact_type_name = {'EQUIP_BRACER': 'flower', 'EQUIP_NECKLACE': 'wing', 'EQUIP_SHOES': 'clock',
                          'EQUIP_RING': 'cup', 'EQUIP_DRESS': 'crown'}  # JSONの値の呼び方を設定
    artifacts = {
    ...
    with self.ArtifactFont() as Font:  # フォントファイルを一時フォルダへ展開後、フォントファイルを読み込み
      ...
      try:
        DR.rounded_rectangle(
          (60 + character_levellength + 5, 74, 77 + character_levellength + character_friendShiplength, 102), radius=0,
          outline=None, width=0)  # 好感度をレベルの文の長さに間隔を開けてから記入
      except:
        pass
      ...
      Disper = ['会心率', '会心ダメージ', '攻撃パーセンテージ', '防御パーセンテージ', 'HPパーセンテージ',
                '水元素ダメージ', '物理ダメージ',
                '風元素ダメージ', '岩元素ダメージ', '炎元素ダメージ', '与える治癒効果', '与える治療効果',
                '雷元素ダメージ',
      ...
      for k, v in character_status_json.items():  # 元素ダメージごとの書き込み
        if k in ['氷元素ダメージ', '水元素ダメージ', '岩元素ダメージ', '草元素ダメージ', '風元素ダメージ',
                 '炎元素ダメージ', '物理ダメージ',
      ...
      OptionMap = {'攻撃パーセンテージ': '攻撃%', '防御パーセンテージ': '防御%', '元素チャージ効率': '元チャ効率',
                   'HPパーセンテージ': 'HP%'}
      ...
      try:
        ...
        import subprocess
        try:
          subprocess.run('open {}'.format(os.path.join(os.getcwd(), 'ArtifacterImageOutput', 'buildcard.png')),
                         shell=True)
        except:
          pass
      except:
  ...
  def main(self):
    if len(self.uid) == 9:  # UIDが9桁なら
      player_info = self.playerinfo()  # プレイヤー情報を取得
      if 'showAvatarInfoList' in player_info:
        try:
          try:  # アイコンデータを取得。エラーなら違う方式で読み込み
            icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}.png'.format(
              self.character_pfps_json()['{}'.format(self.playerinfo()['profilePicture']['id'])]['iconPath'].replace(
                '_Circle', '')), headers={'User-Agent': UsrAgn})).read()
          except:
            icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}.png'.format(
              self.character_pfps_json()['{}'.format(self.playerinfo()['profilePicture']['id'])]['iconPath']),
              headers={'User-Agent': UsrAgn})).read()
        except:
          icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format('{}.png'.format(
            self.characters_json['{}'.format(self.playerinfo()['profilePicture']['avatarId'])]['SideIconName'].replace(
              'UI_AvatarIcon_Side_', 'UI_AvatarIcon_'))), headers={'User-Agent': UsrAgn})).read()
  ...
  def artifact_grades(self, fname):
    if fname == 'SS':
      return None  # _=base64String=_
    if fname == 'S':
      return None  # _=base64String=_
    if fname == 'A':
      return None  # _=base64String=_
  ...
  def artifact_emotes(self, fname):
    if fname == 'CdReduction_primary.svg':
      return None  # _=base64String=_
    if fname == 'HP':
      return None  # _=base64String=_
    if fname == 'HPパーセンテージ':
      return None  # _=base64String=_
    if fname == 'SHIELD_COST_MINUS_RATIO.svg':
      return None  # _=base64String=_
    if fname == '与える治療効果':
      return None  # _=base64String=_
    if fname == '与える治癒効果':
  ...
  def artifact_base_image(self, fname):
    if fname == '岩':
      return None  # _=base64String=_
    if fname == '水':
      return None  # _=base64String=_
  ...
  def artifact_assets(self, fname):
    if fname == 'CharacterMask':
      return None  # _=base64String=_
    if fname == 'CharacterBlur':
      return None  # _=base64String=_
    if fname == 'ja-jp.ttf':
      return None  # _=base64String=_
    if fname == 'Love':
  ...
  def artifact_constellation(self, fname):
    if fname == '岩':
      return None  # _=base64String=_
    if fname == '岩LOCK':
      return None  # _=base64String=_
  ...
  def artifact_rarelity(self, fname):
    if fname == '1':
      return None  # _=base64String=_
    if fname == '2':
      return None  # _=base64String=_
  ...
  class ArtifactFont(object):
    def __init__(self):
      self.filename = 'tmp.ttf'
      self.tempFolder = tempfile.gettempdir()
      self.tempPath = os.path.join(self.tempFolder, self.filename)
      with open(self.tempPath, 'wb') as fontF:
        # fontF.write(io.BytesIO(_=base64String=_).getvalue())
        pass
  ...
  def _override_navigationController(self):
    ...
    def doneButtonTapped_(_self, _cmd, _sender):
      ...
      visibleViewController.dismissViewControllerAnimated_completion_(
        True, None)
      with open(os.path.join(os.getcwd(), '_kill'), 'w', encoding='utf-8') as f:
        f.write('1')
...
class TopViewController(_ViewController):
  ...
  def override(self):
    ...
    def setupHeaderStack(_self, _cmd):
      ...
      self.header_icon_img = UIImage.imageWithData_(None)  # (_=base64String=_)
    ...
    def setupUIDStack(_self, _cmd):
      ...
      placeholder = NSAttributedString.alloc().initWithString_('UIDを入力してください...')
    ...
    def setupUserRankStack(_self, _cmd):
      ...
      self.username_key_label = ObjcLabel.new(text='ユーザー名:')
    ...
    def setupButton(_self, _cmd):
      ...
      self.make_button = ObjcButton.new(title='作成する')
      ...
      if _pre_view:
        activateConstraints.append(
          _view.topAnchor().constraintEqualToAnchor_constant_(
            _pre_view.bottomAnchor(), 30.0))
      else:
        activateConstraints.append(_view.topAnchor().constraintEqualToAnchor_constant_(view.topAnchor(), 9.5))
...
def _start():
  ...
  def target(_file):
    ...
    while True:
      try:
        if open(os.path.join(os.getcwd(), '_kill'), 'r', encoding='utf-8').read() == '1':
          try:
            os.remove(os.path.join(os.getcwd(), '_kill'))
          except:
            pass
          os.system('exit')
          sys.exit(0)
  ...
  with concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 9999) as th:
    th.submit(target, _file)
    th.shutdown()
