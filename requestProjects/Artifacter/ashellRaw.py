...
class objc_util_loader(object): # objc_util installer
    def __int__(self):
        ...
        shutil.move(os.path.join(os.getcwd(), 'objc_util.py'), os.path.join(os.environ.get('HOME'), 'Library', 'lib', 'python3.11', 'site-packages', 'objc_util.py'))
  ...
  def create_buildcard(self, avaterinfo, score_state):
    ...
    with self.ArtifactFont() as Font:  # フォントファイルを一時フォルダへ展開後、フォントファイルを読み込み
      ...
      try:
        ...
        import subprocess
        try:
          subprocess.run('open {}'.format(os.path.join(os.getcwd(), 'ArtifacterImageOutput', 'buildcard.png')), shell=True)
        except:
          pass
      except:
        pass
      ...
      if 'showAvatarInfoList' in player_info:
        try:
          try:  # アイコンデータを取得。エラーなら違う方式で読み込み
            icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}.png'.format(self.character_pfps_json()['{}'.format(self.playerinfo()['profilePicture']['id'])]['iconPath'].replace('_Circle', '')), headers={'User-Agent': UsrAgn})).read()
          except:
            icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}.png'.format(self.character_pfps_json()['{}'.format(self.playerinfo()['profilePicture']['id'])]['iconPath']), headers={'User-Agent': UsrAgn})).read()
        except:
          icon_data = urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format('{}.png'.format(self.characters_json['{}'.format(self.playerinfo()['profilePicture']['avatarId'])]['SideIconName'].replace('UI_AvatarIcon_Side_', 'UI_AvatarIcon_'))), headers={'User-Agent': UsrAgn})).read()
