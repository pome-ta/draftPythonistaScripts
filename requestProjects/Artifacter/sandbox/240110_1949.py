...
ArtifactUISheet_b64 = Path('./assets/base64/ArtifactUISheet.b64')
ArtifactUISheet = base64.b64decode(ArtifactUISheet_b64.read_text())
'''
...
class Artifacter(object):
  ...
  def create_buildcard(self, avaterinfo, score_state):
    ...
    try:
      ...
      character_icon = '{}.png'.format('{}'.format(
        self.characters_json[character_id]['SideIconName']).replace(
          'UI_AvatarIcon_Side_', 'UI_Gacha_AvatarImg_'))  # キャラクターの画像
    try:
    ...
    character_element_buff_value = '{}'.format(
      Decimal(avaterinfo['fightPropMap'][character_element_max_buff_key] *
              100).quantize(Decimal('0.1'), rounding=ROUND_HALF_UP))
    ...
    try:
      weapon_sub_name = self.locale_jp[list(
        filter(lambda item: item['appendPropId'] != 'FIGHT_PROP_BASE_ATTACK',
               weapon['flat']['weaponStats']))[0]['appendPropId']]
    except:
...
v = ui.load_view('./assets/ArtifacterUIFile.pyui')
IconView_b64 = Path('./assets/base64/IconView.b64')
v['IconView'].image = ui.Image.from_data(
  base64.b64decode(IconView_b64.read_text()))
