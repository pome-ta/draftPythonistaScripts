...
class Artifacter(object):
  ...
  def Initialization(self):
    self.locale_jp = concurrent.futures.ThreadPoolExecutor().submit(
      self.locale).result()  # データの表記名データをEnka.Network公式Githubから取得
    self.characters_json = concurrent.futures.ThreadPoolExecutor().submit(
      self.character_json).result()  # キャラクターデータをEnka.Network公式Githubから取得
    ...
    for percent in [ja for ja in LJsn['ja'] if 'PERCENT' in ja]:
      if '力' in LJsn['ja'][percent]:
        ...
        LJsn['ja'][percent] = LJsn['ja'][percent].replace(
          LJsn['ja'][percent][-1], '{}パーセンテージ'.format(LJsn['ja'][percent][-1]))
    return LJsn['ja']
  ...
  def playerinfo(self):  # プレイヤー情報の一部を取得
    player_infomation = self.player_data[
      'playerInfo']  # 取得したデータから「playerInfo」を取得
    if 'showAvatarInfoList' in player_infomation:  # 取得した「playerInfo」から「showAvatarInfoList」がある場合は以下を実行
      for avater in player_infomation[
          'showAvatarInfoList']:  # 取得したプレイヤーデータからキャラクター名を日本語へ変換
  ...
  def create_buildcard(self, avaterinfo, score_state):
    ...
    try:  # キャラクタースキルセットIDを判別。
      if len('{}'.format(
          avaterinfo['skillDepotId'])) == 3:  # キャラクタースキルセットIDが３なら以下を実行
        character_id = '{}-{}'.format(
          avaterinfo['avatarId'], avaterinfo['skillDepotId']
        )  # キャラクタースキルセットIDは[avatarid]-[skillDepotId]という形式で取得できる
    ...
    character_name = self.locale_jp['{}'.format(
      self.characters_json[character_id]['NameTextMapHash'])]
    # キャラクター名
    try:
      character_icon = '{}.png'.format(
        self.characters_json[character_id]['Costumes']['{}'.format(
          avaterinfo['costumeId'])]['sideIconName'].replace(
            'UI_AvatarIcon_Side_', 'UI_Costume_'))
      # キャラクターがコスチュームを付けていた場合の対応
    except:
      #character_icon ='{}.png'.format('{}'.format(self.characters_json[character_id]['SideIconName']).replace('UI_AvatarIcon_Side_',　'UI_Gacha_AvatarImg_'))
      pass

    try:
      character_const = len(
        avaterinfo['talentIdList']
      ) - 1  # キャラクターとつ数(画像上半分の真ん中にある細長い長方形の下半分の鍵アイコンが入ってる部分)
    except:
      character_const = -1
    character_const_icons = [
      '{}.png'.format(cicon)
      for cicon in self.characters_json[character_id]['Consts']
    ]  # キャラクター凸画像(画像上半分の真ん中にある細長い長方形の下半分の鍵アイコンが入ってる部分のアイコン画像)
    character_level = '{}'.format(
      avaterinfo['propMap']['4001']['val'])  # キャラクターのレベル
    ...
    character_charge_efficiency = Decimal(
      avaterinfo['fightPropMap']['23'] * 100).quantize(
        Decimal('0.1'), rounding=ROUND_HALF_UP)  # キャラクターの元素チャージ効率
    ...
    for sk in character_element_buff:  # キャラクターの元素ダメージを決める
      if avaterinfo['fightPropMap'][
          sk] > character_element_max_value:  # キャラクターの元素番号が30よりおおきいか
    ...
    character_element_buff_value = '{}'.format(
      Decimal(avaterinfo['fightPropMap'][character_element_max_buff_key] *
              100).quantize(Decimal('0.1'),
                            rounding=ROUND_HALF_UP))  # 元素ダメージの値を設定
    ...
    try:
      weapon_sub_value = list(
        filter(lambda item: item['appendPropId'] != 'FIGHT_PROP_BASE_ATTACK',
               weapon['flat']['weaponStats']))[0]['statValue']  # 武器のプロパティを設定
    except:
      weapon_sub_value = '1'
      ...
      if not artifact == None:  # 聖遺物情報があるかないか。あるなら以下実行
        ...
        for i, s in enumerate(_Artifact['flat']['reliquarySubstats']):
          ...
          if artifact['suboption'][i]['optionName'] in [
              artifact_state[score_state], '会心ダメージ'
          ]:
    ...
    with self.ArtifactFont() as Font:  # フォントファイルを一時フォルダへ展開後、フォントファイルを読み込み
      ...
      BaseImage = Image.alpha_composite(BaseImage,
                                        weapon_image_paste)  # ベース画像と武器の画像を合成
      # 武器のレアリティ画像の編集
      ...
      BaseImage = Image.alpha_composite(
        BaseImage, weapon_rarelity_image_paste)  # ベース画像に武器のレアリティ画像を貼り付け
      character_talent_base = Image.open(
        io.BytesIO(self.artifact_assets('TalentBack')))  # 天賦のベース画像
      character_talent_paste = Image.new(
        'RGBA', BaseImage.size, (255, 255, 255, 0))  # 天賦のベース画像を合成するためのベース画像
      character_talent_base = character_talent_base.resize(
        (int(character_talent_base.width / 1.5),
         int(character_talent_base.height / 1.5)))  # 天賦のベース画像をリサイズ
      for i, t in enumerate(character_skill_icons):  # 天賦ベース画像とスキルアイコンとの合成
        ...
        _talent_object = Image.alpha_composite(
          character_talent_base, _talent_paste)  # 天賦画像と新規天賦ベース画像を合成
        character_talent_paste.paste(_talent_object,
                                     (1005, 10 + i * 86))  # 天賦画像を指定の位置で合成
      ...
      character_constellations_paste = Image.new(
        'RGBA', BaseImage.size, (255, 255, 255, 0))  # 凸数の新しいベース画像を作成
      ...
      BaseImage = Image.alpha_composite(
        BaseImage, character_constellations_paste)  # ベース画像へ凸数を反映
      ...
      character_friendShipicon = character_friendShipicon.resize(
        (int(character_friendShipicon.width *
             (24 / character_friendShipicon.height)), 24))  # 好感度アイコンをリサイズ
      character_friendShipicon_mask = character_friendShipicon.copy(
      )  # 好感度アイコンのマスクを作成
      BaseImage.paste(character_friendShipicon,
                      (74 + int(character_levellength), 108),
      ...
      character_status_json = {
        ...
        '会心ダメージ': character_critical_damage_rate,
      ...
      Disper = [
        '会心率', '会心ダメージ', '攻撃パーセンテージ', '防御パーセンテージ', 'HPパーセンテージ', '水元素ダメージ',
        '物理ダメージ', '風元素ダメージ', '岩元素ダメージ', '炎元素ダメージ', '与える治癒効果', '与える治療効果',
        '雷元素ダメージ', '氷元素ダメージ', '草元素ダメージ', '与える治癒効果', '元素チャージ効率'
      ]
      StateOption = ('HP', '攻撃力', '防御力', '元素熟知', '会心率', '会心ダメージ', '元素チャージ効率')
      for k, v in character_status_json.items():  # 元素ダメージごとの書き込み
        if k in [
            '氷元素ダメージ', '水元素ダメージ', '岩元素ダメージ', '草元素ダメージ', '風元素ダメージ', '炎元素ダメージ',
        ...
        try:
          ...
          ElementOpIcon = Image.open(io.BytesIO(
            self.artifact_emotes(k))).resize((40, 40))
      ...
      for i, part in enumerate(['flower', "wing", "clock", "cup",
        ...
        if artifact_icons == '':
          ...
          try:
            ...
            MainOptionLen = DR.textlength(
              MainOption, font=artifact_font(29))  # 聖遺物のメインオプションの文字長を計算
          try:
          ...
          MainOption_icon = Image.open(
            io.BytesIO(
              self.artifact_emotes(MainOption))).convert('RGBA').resize(
          ...
          artifactLevelLen = DR.textlength('+{}'.format(
            artifact_details['level']), artifact_font(21))  # 聖遺物のレベルの文字の長さを計算
          DR.rounded_rectangle(
          ...
          for a, arts in enumerate(artifact_details['suboption']):
            ...
            artifact_suboption_icon = Image.open(
              io.BytesIO(self.artifact_emotes(artifact_suboption))).convert(
                'RGBA').resize((30, 30))
    ...
    if len(self.uid) == 9:  # UIDが９桁なら
      ...
      if 'showAvatarInfoList' in player_info:
        try:  # アイコンデータを取得。エラーなら違う方式で読み込み
          v['IconView'].image = ui.Image.from_data(
            urllib.request.urlopen(
        ...
        try:
          ...
          pass
        if not ''.join(
            v['characterList'].data_source.items) == '':  # キャラクターデータが既にあるなら削除
          try:
            CL = [0]
            for _i in CL:
        ...
        v['characterList'].data_source.items = [
          '{} \tLv{}'.format(avatar['name'], avatar['level'])
          for index, avatar in enumerate(avatar_lists)
        ]  # キャラクターデータリストをリストに反映
...
ui.Image.from_data(base64.b64decode())
