...
kDroppedFrameReason = ObjCInstance(
  c_void_p.in_dll(c, 'kCMSampleBufferAttachmentKey_DroppedFrameReason'))
...
def CGBitmapContextCreate(baseAddress, width, height, param_0, bytesPerRow,
                          colorSpace, flags):
  ...
  func.restype = ctypes.c_void_p
  result = func(baseAddress,
                int(width),
                int(height),
                int(param_0),
                int(bytesPerRow), ObjCInstance(colorSpace).ptr, flags)
...
def UIImageFromSampleBuffer(buffer):
  ...
  context = CGBitmapContextCreate(baseAddress, width / 4, height / 4, 8,
                                  4 * width, colorSpace, flags)
...
def detect_faces(samplebuffer):
  ...
  face_detector = CIDetector.detectorOfType_context_options_(
    'CIDetectorTypeFace', None, options)
  rectangle_detector = CIDetector.detectorOfType_context_options_(
    'CIDetectorTypeRectangle', None, options)
...
def compute_fps():
  ...
  if view is not None:
    ...
    error_str = ''
    set_label('fps={:0.1f} proc:{:9d} faces:{} rects:{} {} {}'.format(
...
def set_frame_rate(inputDevice, captureSession, desired_fps):
  ...
  if (desired_fps) <= supported_rates.maxFrameRate(
  ) and desired_fps >= supported_rates.minFrameRate():
    ...
    if err_ptr:
      raise ()
    inputDevice.setActiveVideoMinFrameDuration_((desired_intervalnum, 600, 0,
                                                 0))
