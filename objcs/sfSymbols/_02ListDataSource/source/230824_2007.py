...
def _view_from_dict(view_dict, f_globals, f_locals, verbose=True):
  ...
  if custom_class_str:
    try:
      ...
      if inspect.isclass(CustomViewClass) and issubclass(
          CustomViewClass, View):
        ViewClass = CustomViewClass
      elif verbose:
        sys.stderr.write('Warning: Invalid custom view class "%s"' %
                         (custom_class_str, ))
    except Exception as e:
      if verbose:
  ...
  if classname == 'NavigationView':
    ...
    root_view.background_color = _str2color(attrs.get('background_color'),
                                            'white')
  ...
  if classname == 'Label':
    ...
    v.autocorrection_type = CORRECTION_TYPES[attrs.get('autocorrection_type',
                                                       'default')]
    v.spellchecking_type = CORRECTION_TYPES[attrs.get('spellchecking_type',
                                                      'default')]
    ...
    v.content_size = int(attrs.get('content_width',
                                   '0')), int(attrs.get('content_height', '0'))
...
def _view_to_dict(view):
  ...
  if isinstance(view, TableView):
    ...
    if isinstance(view.data_source, ListDataSource) and isinstance(
      ...
      _set_action_name(attrs, data_source.edit_action,
                       'data_source_edit_action')
      _set_action_name(attrs, data_source.accessory_action,
                       'data_source_accessory_action')
