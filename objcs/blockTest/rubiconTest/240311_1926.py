...
class ObjcTableViewController:
  ...
  def add_extensions(self):
    ...
    def configureDropDownProgrammaticButton_(_self, _cmd, _button):
      ...
      handler_block = ObjCBlock(menuHandler_, **handler_kwargs)
      
      #pdbg.state(handler_block.func)

      #block_ptr = ObjCBlockPointer(handler_block, **handler_kwargs)

      #bp = ObjCBlockPointer(menuHandler_,restype=None,argtypes=[ctypes.c_void_p, ctypes.c_void_p])

      _handler = None

      action = UIAction.actionWithTitle_image_identifier_handler_(
        pylocalizedString('ItemTitle'), None, 'item', _handler)
