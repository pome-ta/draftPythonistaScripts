...
class ObjCMethod:
  ...
  def __init__(self, method):
    ...
    self.restype, *self.imp_argtypes = ctypes_for_method_encoding(
      self.encoding)
...
class objc_method:
  ...
  def __call__(self, objc_self, objc_cmd, *args):
    ...
    if self.encoding[0] is not None and issubclass(self.encoding[0],
                                                   (objc_id, ObjCInstance)):
  ...
  def protocol_register(self, proto_ptr, attr_name):
    name = attr_name.replace("_", ":")
    types = b"".join(
      encoding_for_ctype(ctype_for_type(tp)) for tp in self.encoding)
    libobjc.protocol_addMethodDescription(proto_ptr, SEL(name), types, True,
                                          True)
...
class objc_property:
  ...
  def class_register(self, class_ptr, attr_name):
    ...
    libobjc.class_addProperty(class_ptr, ensure_bytes(attr_name), attrs,
                              len(attrs))
  ...
  def protocol_register(self, proto_ptr, attr_name):
    attrs = self._get_property_attributes()
    libobjc.protocol_addProperty(proto_ptr, ensure_bytes(attr_name), attrs,
                                 len(attrs), True, True)
...
class objc_rawmethod:
  ...
  def protocol_register(self, proto_ptr, attr_name):
    raise TypeError("Protocols cannot have method implementations, "
...
class ObjCInstance:
  ...
  def objc_class(self):
    ...
    try:
      return super(ObjCInstance,
                   type(self)).__getattribute__(self, "_objc_class")
    except AttributeError:
      ...
      #    available. Refs #249.
      super(ObjCInstance,
            type(self)).__setattr__(self, "_objc_class",
                                    ObjCClass(libobjc.object_getClass(self)))
      return super(ObjCInstance,
                   type(self)).__getattribute__(self, "_objc_class")
  ...
  def __new__(cls, object_ptr, _name=None, _bases=None, _ns=None):
    ...
    with ObjCInstance._instance_lock:
      try:
        ...
        if cls == ObjCInstance or isinstance(cls, ObjCInstance):
          ...
          current_class_name = libobjc.class_getName(
            libobjc.object_getClass(object_ptr)).decode("utf-8")
          if (current_class_name != cached_class_name
      ...
      super(ObjCInstance, type(self)).__setattr__(self, "_as_parameter_",
                                                  object_ptr)
      super(ObjCInstance, type(self)).__setattr__(self, "_needs_release",
                                                  False)
      if isinstance(object_ptr, objc_block):
        super(ObjCInstance, type(self)).__setattr__(self, "block",
                                                    ObjCBlock(object_ptr))
...
class ObjCClass(ObjCInstance, type):
  ...
  def protocols(self):
    ...
    return tuple(
      ObjCProtocol(protocols_ptr[i]) for i in range(out_count.value))
    ...
    if bases is None and attrs is None:
      ...
      if isinstance(name_or_ptr, (bytes, str)):
        ...
        if not issubclass(cls,
                          ObjCMetaClass) and libobjc.class_isMetaClass(ptr):
          return ObjCMetaClass(ptr)
    else:
      ptr, name, attrs = cls._new_from_class_statement(name_or_ptr,
                                                       bases,
                                                       attrs,
                                                       protocols=protocols)
  ...
  def _load_methods(self):
    if self.methods_ptr is not None:
      raise RuntimeError(
        f"{self}._load_methods cannot be called more than once")
...
def py_from_ns(nsobj):
  ...
  if nsobj.isKindOfClass(NSDecimalNumber):
    ...
    if objc_type == b"B":
      ...
      raise TypeError(f"NSNumber containing unsupported type {objc_type!r} "
...
def ns_from_py(pyobj):
  ...
  if pyobj is None or isinstance(pyobj, ObjCInstance):
    ...
    return ObjCInstance(
      NSString.stringWithUTF8String_(pyobj.encode("utf-8"),
                                     convert_result=False))
  elif isinstance(pyobj, bytes):
    return ObjCInstance(NSData.dataWithBytes(pyobj, length=len(pyobj)))
  elif isinstance(pyobj, decimal.Decimal):
    return ObjCInstance(
      NSDecimalNumber.decimalNumberWithString_(pyobj.to_eng_string(),
                                               convert_result=False))
    ...
    return ObjCInstance(NSNumber.numberWithDouble_(pyobj,
                                                   convert_result=False))
  else:
    raise TypeError(
      ...
      if isinstance(name_or_ptr, (bytes, str)):
        ...
        if ptr.value is None:
          raise ValueError("Cannot create ObjCProtocol for nil pointer")
            ...
            ptr, "isKindOfClass:", Protocol, restype=c_bool,
            argtypes=[objc_id]):
          raise ValueError(
...
def create_block_descriptor_struct(has_helpers, has_signature):
  ...
  return type("ObjCBlockDescriptor", (Structure, ),
              {"_fields_": descriptor_fields})


def cast_block_descriptor(block):
  descriptor_struct = create_block_descriptor_struct(block.has_helpers,
                                                     block.has_signature)
...
class ObjCBlock:
  ...
  def __init__(self, pointer, restype=AUTO, *argtypes):
    ...
    self.signature = (self.descriptor.contents.signature
                      if self.has_signature else None)
    if restype is AUTO:
      if argtypes:
  ...
  def __call__(self, *args):
    ...
    for i, argtype in enumerate(self.invoke_argtypes[1:]):
      if (isinstance(args[i], Structure) and issubclass(argtype, Structure)
        ...
        if anon_fields != arg_fields:
          raise TypeError(f"Expected structure with field types {anon_fields} "
