...
class _NavigationController(_Controller):
    ...
    def create_navigationControllerDelegate(self):
        ...
        def navigationController_willShowViewController_animated_(
            ...
            self.willShowViewController(navigationController, viewController, _animated)
    ...
    # @on_main_thread
    def _init_controller(self, vc: UIViewController, is_main_thread: bool = False):
        def __initialize():
            self._override_controller()
    ...
    @classmethod
    def new(
        cls, viewController: UIViewController, is_main_thread: bool = False
    ) -> ObjCInstance:
        _cls = cls()
...
class TopNavigationController(PlainNavigationController):
    ...
    def willShowViewController(
        ...
        done_btn = UIBarButtonItem.alloc().initWithBarButtonSystemItem_target_action_(
...
class SecondViewController(_ViewController):
    def __init__(self, *args, **kwargs):
        ...
        self.table_items = (
            [
                key
                for key, value in self.symbol_categories.items()
                if self.select in value
            ]
            if self.select != "all"
            else list(self.symbol_categories.keys())
        )
...
def is_ignore(path: Path) -> bool:
    return (
        not path.is_file() or path.suffix in IGNORE_SUFFIXS or path.name in IGNORE_NAMES
    )
...
if __name__ == "__main__":
    ...
    paths = {
        ...
        for bundle in Path(CoreGlyphs_ROOT).iterdir()
        if not is_ignore(bundle)
    }
