...
def parse_types(type_encoding):
  """Take an Objective-C type encoding string and convert it to a tuple of (restype, argtypes) appropriate for
  objc_msgSend() """
  ...
  def get_type_for_code(enc_str):
    ...
    if enc_str[0] == '^':
      if re.match(r'\^\{\w+=#?\}', enc_str):
        # pointer to opaque type, e.g. CGPathRef, CGImageRef...
        return c_void_p
      else:
...
def get_possible_method_names(name, args, kwargs):
  ...
  if len(args) == 0 and len(kwargs) == 0:
    ...
    for perm in permutations:
      ...
      if len(args) == 0:
        name2 = name + \
            'With%s_' % (_first_letter_cap(perm[0]),) + '_'.join(perm[1:])
        if len(perm) > 1:
          name2 += '_'
...
def resolve_cls_method(cls, name, args, kwargs):
  # Return (method_name, ordered_kwargs), or raise an AttributeError if either no method could be found,
  # or if the mapping was ambiguous.
...
class ObjCClass(object):
  """Wrapper for a pointer to an Objective-C class; acts as a proxy for calling Objective-C class methods. Method
  calls are converted to Objective-C messages on-the-fly -- this is done by replacing underscores in the method
  name with colons in the selector name, and using the selector and arguments for a call to the low-level
  objc_msgSend function in the Objective-C runtime. For example, calling
  `NSDictionary.dictionaryWithObject_forKey_(obj, key)` (Python) is translated to `[NSDictionary
  dictionaryWithObject:obj forKey:key]` (Objective-C). If a method call returns an Objective-C object,
  it is wrapped in an ObjCInstance, so calls can be chained (ObjCInstance uses an equivalent proxy mechanism). """
...
class ObjCClassMethod(object):
  """Wrapper for an Objective-C class method. ObjCClass generates these objects automatically when accessing an
  attribute, you typically don't need use this class directly. """
...
class ObjCInstanceMethodProxy(object):
  """A proxy for an ObjCInstanceMethod that is resolved to an actual method when calling it. A proxy may represent
  different methods, depending on the keyword arguments that are passed (and used to construct an ObjC selector
  from the call). """

  def __init__(self, obj, name):
    self.obj = weakref.ref(obj)
    self.name = name
    # Determining the method for a given combination of args/kwargs can be expensive, so it's cached by
    # 'num_args/sorted_kwarg_keys'. The cache contains a pair of (ObjCClassMethod, ordered_kwarg_keys).
    self.method_cache = {}
...
class ObjCInstanceMethod(object):
  """Wrapper for an Objective-C instance method. ObjCInstance generates these objects automatically when accessing
  an attribute, you typically don't need to use this class directly. """
...
def ns(py_obj):
  """Convert common Python objects to their ObjC equivalents, i.e. str => NSString, int/float => NSNumber,
  list => NSMutableArray, dict => NSMutableDictionary, bytearray => NSData, set => NSMutableSet. Nested structures
  (list/dict/set) are supported. If an object is already an instance of ObjCInstance, it is left untouched.
  """
