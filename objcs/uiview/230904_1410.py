...
# [Pythonista/omz/AddWebTab_omz.py at master · tdamdouni/Pythonista](https://github.com/tdamdouni/Pythonista/blob/master/omz/AddWebTab_omz.py)
...
def main():
  ...
  share_item = UIBarButtonItem.alloc().initWithImage_style_target_action_(
    UIImage.imageNamed_('Action'), 0, vc, sel('share'))
  ...
  webView.loadRequest_(
    NSURLRequest.requestWithURL_(nsurl('http://www.google.com')))
  ...
  # objc_utilには、setAutocapitalizationType:の通常のメソッド名の変換を失敗させるバグがあるようです(プロパティがプロトコルで定義されていることに関係している可能性があります。)
    ...
    if res:
      ...
      view.loadRequest_(
        NSURLRequest.requestWithURL_(
          nsurl('http://www.google.com/search?q=' + urllib.quote(term))))
